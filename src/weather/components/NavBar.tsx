...
export const ResponsiveNavBar = () => {
  ...
  return (
    <AppBar position="static" sx={{ backgroundColor: "white", width: "100px", backgroundColor: "rgba(0, 0, 0, 0.1)", borderRadius:3 }}>
      <Toolbar disableGutters sx={{ display: "flex", flexDirection: "column", width: "100px", marginTop:"40px" }}>
        <IconButton
          size="large"
        ...
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: 'center' }}>
          {pages.map((page) => (
            <Button
              ...
              sx={{ my: 2, color: "rgba(0, 0, 0, 0.6)", display: 'flex', flexDirection: 'column', alignItems: 'center' }}
            >
              {page.icon}
              <Typography variant="caption" sx={{ mt: 1 , fontSize:"14px"}}>{page.label}</Typography>
            </Button>
          ))}
        </Box>
      </Toolbar>
